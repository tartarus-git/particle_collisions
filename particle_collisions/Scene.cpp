...
void Scene::setupHelperVectors() {
	lastParticleCollisions.resize(particleCount);
	earliestCollisions.resize(particleCount);
	for (int i = 0; i < particleCount; i++) {
		lastParticleCollisions[i] = i;					// Last collision for every particle was itself. Signals that no last collision exists yet.
		earliestCollisions[i] = 1;						// Earliest collisions are all at max possible value to start.
	}
}
...
void Scene::findCollision(size_t aIndex, size_t bIndex) {
	...
	alpha.vel *= 1 - earliestCollisions[aIndex];
	beta.vel *= 1 - earliestCollisions[bIndex];
